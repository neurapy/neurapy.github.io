...
class BestModelCheckpoint(Callback):
    ...
    def on_train_end(self):
        ...
        if self.verbose > 0:
            print(f"Training completed. Best {self.monitor} = {self.best:.2e} @ {self.best_epoch}")
    ...
    def get_monitor_value(self):
        if self.monitor == "train loss":
            ...
            result = (
                sum(train_loss)
                if isinstance(train_loss, Iterable) and not isinstance(train_loss, (str, bytes))
                else train_loss
            )
            ...
            result = (
                sum(test_loss)
                if isinstance(test_loss, Iterable) and not isinstance(test_loss, (str, bytes))
                else test_loss
            )
        else:
...
class EvalMetricCallback(Callback):
    ...
    def on_epoch_end(self):
        if any(self.model.train_state.loss_test != self.last_test_loss_state):
            ...
            valid_loss = (
                sum(valid_loss)
                if isinstance(valid_loss, Iterable) and not isinstance(valid_loss, (str, bytes))
                else valid_loss
            )
...
class WandbCallbackPlots(WandbCallback):
    ...
    def plot(self):
        ...
        y_pred = self.model.predict(self.model.data.holdout_test_x)
        residuals = self.model.predict(self.model.data.holdout_test_x, operator=self.model.data.pde)
        title = f"Epoch {epoch}"

        fig = self.plotting_func(self.model.data.holdout_test_x, y_true, y_pred, residuals, title)
