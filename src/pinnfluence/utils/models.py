...
class ModelWrapper(torch.nn.Module):
    def __init__(
        ...
        assert include_pde or bcs is not None, "At least one of PDEs or BCs must be included."
            ...
            for i, bc in enumerate(self.bcs):
                ...
                if not isinstance(bc, dde.icbc.PeriodicBC):
                    if isinstance(bc, dde.icbc.IC):
                        bc_mask = torch.tensor(bc.on_initial(x_np, bc.geom.on_initial(x_np))).bool()
                    else:
                    ...
                    bc_mask = torch.tensor(bc.on_boundary(x_np, bc.geom.on_boundary(x_np))).bool()
...
class ModifiedMLP(dde.nn.NN):
    ...
    def __init__(self, layer_sizes, activation, kernel_initializer, regularization=None):
        ...
        if len(layer_sizes) < 3:
            raise ValueError("layer_sizes must be at least [in_dim, hidden_dim, out_dim].")

        hidden_dims = layer_sizes[1:-1]
        if len(set(hidden_dims)) != 1:
            raise ValueError("All hidden layers must have the same width for ModifiedMLP.")
        ...
        self.linear_u = torch.nn.Linear(self.in_dim, self.hidden_dim, dtype=dde.config.real(torch))
        self.linear_v = torch.nn.Linear(self.in_dim, self.hidden_dim, dtype=dde.config.real(torch))
        ...
        for _ in range(self.num_layers - 1):
            self.linears.append(
                torch.nn.Linear(self.hidden_dim, self.hidden_dim, dtype=dde.config.real(torch))
            )
            init_w(self.linears[-1].weight)
            init_b(self.linears[-1].bias)
