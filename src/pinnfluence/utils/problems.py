...
def allen_cahn_equation_broken(x, y):
    u = y
    du_xx = dde.grad.hessian(y, x, i=0, j=0)
    du_t = dde.grad.jacobian(y, x, j=1)
    # diffusion sign flip
...
def burgers_equation_broken(x, y):
    dy_x = dde.grad.jacobian(y, x, i=0, j=0)
    dy_t = dde.grad.jacobian(y, x, i=0, j=1)
    dy_xx = dde.grad.hessian(y, x, i=0, j=0)
    # diffusion sign flip
...
def diffusion_equation_broken(x, y):
    dy_t = dde.grad.jacobian(y, x, j=1)
    dy_xx = dde.grad.hessian(y, x, j=0)
    # diffusion sign flip
    return (
...
def diffusion_output_transform(x, y):
    return torch.sin(np.pi * x[:, 0:1]) + (1 - x[:, 0:1] ** 2) * (x[:, 1:2]) * y
...
def drift_diffusion_equation_broken(x, y):
    du_x = dde.grad.jacobian(y, x, i=0, j=0)
    du_t = dde.grad.jacobian(y, x, i=0, j=1)
    du_xx = dde.grad.hessian(y, x, i=0, j=0)
    # diffusion sign flip
...
def drift_diffusion_solution(x):
    ...
    return (
        ...
        * np.sin(DRIFT_DIFFUSION_R + DRIFT_DIFFUSION_S * (x_in - DRIFT_DIFFUSION_BETA * t_in))
...
def drift_diffusion_output_transform(x, y):
    ...
    ic_at_x = DRIFT_DIFFUSION_U_00 * torch.sin(DRIFT_DIFFUSION_S * x + DRIFT_DIFFUSION_R)
    # IC at x_start
    ...
    g_start_t = (
        DRIFT_DIFFUSION_U_00
        * torch.sin(
            DRIFT_DIFFUSION_S * (current_x_start_val - DRIFT_DIFFUSION_BETA * t) + DRIFT_DIFFUSION_R
        )
    ...
    g_end_t = (
        DRIFT_DIFFUSION_U_00
        * torch.sin(
            DRIFT_DIFFUSION_S * (current_x_end_val - DRIFT_DIFFUSION_BETA * t) + DRIFT_DIFFUSION_R
        )
        * torch.exp(-DRIFT_DIFFUSION_ALPHA * DRIFT_DIFFUSION_S**2 * t)
    )
    ...
    spatial_bc_contribution = (
        weight_factor_start * term_g_minus_ic_start + weight_factor_end * term_g_minus_ic_end
    )
...
# NAVIER STOKES
# see https://github.com/aleks-krasowski/PINNfluence/blob/main/train.py
...
def navier_stokes_equation(x, y):
    ...
    x_momentum = u * du_x + v * du_y + 1 / NS_RHO * dp_x - NS_NU * (du_xx + du_yy)
    y_momentum = u * dv_x + v * dv_y + 1 / NS_RHO * dp_y - NS_NU * (dv_xx + dv_yy)

    return [continuity, x_momentum, y_momentum]


def create_navier_stokes_geometry():
    """Create the Navier-Stokes geometry with cylinder"""
    geom_space = dde.geometry.Rectangle([0, 0], [NS_L, NS_W])
    cylinder = dde.geometry.Disk(NS_CYLINDER_CENTER, NS_RADIUS)
    geom = geom_space - cylinder

    return geom


def navier_stokes_parabolic_inflow_u(xy):
    y = xy[:, 1:2]
    u = 4 * NS_U_MAX * y * (NS_W - y) / (NS_W**2)
    return u


def navier_stokes_outflow_u(X, y, _):
    u_x = dde.grad.jacobian(y, X, i=0, j=0)
    p = y[:, 2:3]
    return NS_NU * u_x - p


def navier_stokes_outflow_v(X, y, _):
    u_y = dde.grad.jacobian(y, X, i=0, j=1)
    return NS_NU * u_y


def navier_stokes_feature_transform(x):
    x_trafo = x[:, 0:1] / NS_L
    y_trafo = x[:, 1:2] / NS_W
    return torch.cat([x_trafo, y_trafo], dim=1)


navier_stokes_conditions = [
    # bc noslip u
    {
        "target_class": "DirichletBC",
        "func": lambda x: 0,
        "on_boundary": lambda x, on_boundary: (
            ...
            and (
                np.isclose(x[1], 0)
                or np.isclose(x[1], NS_W)
                or (
                    np.sqrt(
                        (x[0] - NS_CYLINDER_CENTER[0]) ** 2 + (x[1] - NS_CYLINDER_CENTER[1]) ** 2
                    )
                    <= NS_RADIUS
                )
            )
    ...
    {
        "target_class": "DirichletBC",
        "func": navier_stokes_parabolic_inflow_u,
    ...
    # bc outflow u
    {
        ...
        "func": navier_stokes_outflow_u,
        "on_boundary": lambda x, on_boundary: on_boundary and np.isclose(x[0], NS_L),
    },
    # bc outflow v
    {
        "target_class": "OperatorBC",
        "func": navier_stokes_outflow_v,
        "on_boundary": lambda x, on_boundary: on_boundary and np.isclose(x[0], NS_L),
    },
]


def navier_stokes_load_testdata():
    ...
    Y = np.vstack([u_target, v_target, p_target]).T
    return coords, Y


def navier_stokes_input_transform(x):
    x_ = x[:, 0:1] / NS_L
    y_ = x[:, 1:2] / NS_W
...
def navier_stokes_equation_nd_broken(x, y):
    """Dimensionless steady incompressible Navier–Stokes"""
    u = y[:, 0:1]  # u*
    v = y[:, 1:2]  # v*
    p = y[:, 2:3]  # p*
    ...
    continuity = du_dx + dv_dy
    x_momentum = v * du_dy + dp_dx - (1.0 / REYNOLDS) * (du_dxx + du_dyy)
    y_momentum = u * dv_dx + v * dv_dy + dp_dy - (1.0 / REYNOLDS) * (dv_dxx + dv_dyy)
    ...
    # no-slip u*
    {
        ...
        "on_boundary": lambda x, on_boundary: (
            ...
            and (
                ...
                or (
                    np.sqrt((x[0] - NS_ND_CYL_CENTER[0]) ** 2 + (x[1] - NS_ND_CYL_CENTER[1]) ** 2)
                    <= NS_ND_RADIUS
                )
            )
        ),
...
NS_CYLINDER_CENTER_CENTER_GEOM = np.array([NS_L / 2, 0.2])
NS_ND_CYL_CENTER_CENTER_GEOM = NS_CYLINDER_CENTER_CENTER_GEOM / NS_L_STAR


def create_navier_stokes_geometry_cylinder_in_center():
    geom_space = dde.geometry.Rectangle([0.0, 0.0], [NS_ND_L, NS_ND_W])
    cylinder = dde.geometry.Disk(NS_ND_CYL_CENTER_CENTER_GEOM, NS_ND_RADIUS)
    return geom_space - cylinder


navier_stokes_conditions_nd_cylinder_in_center = [
    # no-slip u*
    {
        "target_class": "DirichletBC",
        "func": lambda x: 0.0,
        "on_boundary": lambda x, on_boundary: (
            ...
            and (
                ...
                or (
                    np.sqrt(
                        (x[0] - NS_ND_CYL_CENTER_CENTER_GEOM[0]) ** 2
                        + (x[1] - NS_ND_CYL_CENTER_CENTER_GEOM[1]) ** 2
                    )
                    <= NS_ND_RADIUS
                )
            )
...
def poisson_disk_equation_broken(x, y):
    dy_xx = dde.grad.hessian(y, x, i=0, j=0)
    dy_yy = dde.grad.hessian(y, x, i=1, j=1)
...
problems = {
    "allen_cahn": {
        ...
        "equation_broken": allen_cahn_equation_broken,
    ...
    "burgers": {
        ...
        "equation_broken": burgers_equation_broken,
    ...
    "diffusion": {
        ...
        "equation_broken": diffusion_equation_broken,
    ...
    "drift_diffusion": {
        ...
        "equation_broken": drift_diffusion_equation_broken,
    ...
    "navier_stokes": {
        "equation": navier_stokes_equation,
        "output_transform": None,  # no hard-constrained formulation
        "load_testdata": navier_stokes_load_testdata,
        "solution": None,
        "conditions": navier_stokes_conditions,
        "use_geometry": True,
        "create_geometry": create_navier_stokes_geometry,
        "feature_transform": navier_stokes_feature_transform,
    },
    "navier_stokes_nd": {
        "equation": navier_stokes_equation_nd,
        "equation_broken": navier_stokes_equation_nd_broken,
        "output_transform": None,  # no hard-constrained formulation
    ...
    "navier_stokes_nd_cylinder_in_center": {
        ...
        "load_testdata": navier_stokes_load_testdata_nd,
        "solution": None,
        "conditions": navier_stokes_conditions_nd_cylinder_in_center,
        "use_geometry": True,
        "create_geometry": create_navier_stokes_geometry_cylinder_in_center,
        "feature_transform": navier_stokes_input_transform_nd,
    },
    "poisson_disk": {
        "equation": poisson_disk_equation,
        "equation_broken": poisson_disk_equation_broken,
    ...
    "wave": {
        ...
        "equation_broken": wave_equation_broken,
