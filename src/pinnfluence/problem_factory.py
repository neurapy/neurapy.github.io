...
def drop_single_point(data, drop_single_point):
    ...
    if drop_single_point == "none":
        ...
        if isinstance(data.bcs[0], dde.icbc.IC):
            ...
            assert sum(bcs_affected) > 0, "No BCs affected by point? This should not happen"

            data.train_x_all = data.train_x_all[~(data.train_x_all == point_to_drop).all(axis=1)]
            # potentially also removes point from BC -- edge point
            data.train_x_bc = data.train_x_bc[~(data.train_x_bc == point_to_drop).all(axis=1)]
            ...
            raise ValueError("drop_single_point 'IC' is only supported for IC conditions")

    elif drop_single_point == "BC":
        bcs = [bc for bc in data.bcs if isinstance(bc, dde.icbc.BC)]
        if len(bcs) > 0:
            bc_points = data.train_x_bc
            bc_points = np.concatenate([bc.collocation_points(bc_points) for bc in bcs], axis=0)
            ...
            raise ValueError("drop_single_point 'BC' is only supported for BC conditions")
    ...
    if conditions is not None:
        for condition in conditions:
            ...
            if condition_class is dde.icbc.PeriodicBC:
                ...
                cond = {k: v for k, v in cond.items() if k not in ["target_class", "component_x"]}
...
def resample_validation_and_test(
    ...
    if load_testdata is not None:
        ...
        if hasattr(data, "num_domain") and data.num_domain > 0:
            if hasattr(data, "num_initial") and data.num_initial > 0:
                num_test_domain = int(
                    num_test * (1 - (data.num_boundary + data.num_initial) / len(data.train_x))
                )
            else:
                num_test_domain = int(num_test * (1 - data.num_boundary / len(data.train_x)))
...
def construct_problem(
    ...
    if broken:
        equation = problem.get("equation_broken", None)
        if equation is None:
            raise ValueError(f"equation_broken is not provided for problem {problem_name}")
        print(f"Using broken equation for problem {problem_name}")
    ...
    if soft_constrained:
        ...
        if output_transform is not None:
            raise ValueError("output_transform must be None for soft-constrained training")
    else:
        output_transform = problem["output_transform"]
        conditions = None

        if output_transform is None:
            raise ValueError("output_transform must be provided for hard-constrained training")
    ...
    if len(valid_set & test_set) > 0:
        print(f"Test and holdout test data overlap in {len(valid_set & test_set)} points")
