...
describe("plot projection", () => {
  it("scales Drift Diffusion display coordinates to a 1:1 aspect", () => {
    const projection = plotProjectionForManifest(
      { ...baseManifest, problem: "drift_diffusion" },
      { minX: 0, maxX: 2 * Math.PI, minY: 0, maxY: 1 },
    );

    expect(projection.displayBounds).toMatchObject({ minX: 0, maxX: 1, minY: 0, maxY: 1 });
    expect(projectPointToDisplay([Math.PI, 0.5], projection)).toEqual([0.5, 0.5]);
    expect(projection.formatXTick(0)).toBe("0");
    expect(projection.formatXTick(0.5)).toBe("π");
    expect(projection.formatXTick(1)).toBe("2π");
    expect(projection.xTickValues).toEqual([0, 0.5, 1]);
  });
  ...
  it("inverts projected drag regions back to physical Drift Diffusion coordinates", () => {
    ...
    expect(displayEnd[0]).toBeCloseTo(1);
