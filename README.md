...
This repo deploys the Vite webdemo through GitHub Actions. The workflow is
`.github/workflows/pages.yml`.

On every push to `main` or `master`, GitHub automatically:
...
For the `neurapy.github.io` repository, the public URL is:

```text
https://neurapy.github.io/
```

The source data must stay committed in `webdemo/public/data/`. Vite copies it
into `webdemo/dist/data/` during the build. Do not commit `webdemo/dist/`; it is
